...
const scrapEducationDetails = async (page) => {
    ...
    for (const item of detailsItems) {
        ...
        if (itemWithHistory) {
            ...
            const [companyHandle] = await item.$x(".//span[contains(@class, 't-bold')]/span[@aria-hidden]")
            const company = await getText(companyHandle)
            let [positionHandle] = await item.$x("(.//span[contains(@class, 't-normal')]/span[@aria-hidden])[1]")
            let [durationInfoHandle] = await item.$x("(.//span[contains(@class, 't-normal')]/span[@aria-hidden])[2]")
            if (!durationInfoHandle) [positionHandle, durationInfoHandle] = [durationInfoHandle, positionHandle]
            let position = await getText(positionHandle)
            let durationInfo = (await getText(durationInfoHandle))?.split(' · ');
...
export default scrapEducationDetails